...
type FpgaMmapM   =  StateT FpgaPtr IO
...
getStateType :: FpgaM FpgaMmapM a -> FpgaMmapM a
getStateType s = s 

-- Constructor
fpgaState a =  StateT a 


runFpga :: FpgaM FpgaMmapM a -> FpgaPtr -> IO (a,FpgaPtr) 
runFpga s = runStateT s

getState :: FpgaM FpgaMmapM FpgaPtr
getState = fpgaState $  \x -> ( return  (x,x) )

getPtr :: FpgaM FpgaMmapM FpgaPtr
getPtr = getState



type Fpga a b = FpgaM a b

-- | This function handles initialising Fpga memory mapping and
-- evaluates 'Fpga' action.
withOpenFpga :: (FpgaM FpgaMmapM a) -> IO a
withOpenFpga act = do
...
getOffsetPtr :: Page -> Offset -> Fpga FpgaMmapM (Ptr Registry)
...
fpgaWrite :: Page ->  Offset -> Registry -> Fpga FpgaMmapM ()
